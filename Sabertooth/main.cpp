...
int main() {
	...
	points[1].coords[0] = 0.15f;
	points[1].coords[1] = -0.15f;
	points[1].coords[2] = 0.0f;
	points[2].coords[0] = -0.15f;
	points[2].coords[1] = -0.15f;
	...
	/* these are the strings of code for the shader
	...
	/* the fragment shader colors each fragment (pixel-sized area of the
	triangle) */
	...
	GLuint vs, fs, fs2;
	/* GL shader programmer object [combined, to link] */
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (elapsedSeconds > 0) {
			...
			if (fabs(lastPositionX) > 0.8f)
				speedX = -speedX;
			else if (fabs(lastPositionY) > 0.8f)
				speedY = -speedY;
